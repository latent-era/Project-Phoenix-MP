...
```
Look for these 11 tools (8 workflows + 3 cognitive) in your available tools:
...
Cognitive tools (v6.0.0):
- mcp__daem0nmcp__simulate_decision  (temporal scrying)
- mcp__daem0nmcp__evolve_rule        (rule entropy analysis)
- mcp__daem0nmcp__debate_internal    (adversarial council)
```
...
Three standalone reasoning tools for autonomous thought. These are NOT workflow actions - invoke them directly.

### Temporal Scrying - Replay Past Decisions
...
Reconstructs the knowledge state when decision #42 was made, compares with current understanding, reveals what is now known that wasn't known then. Use when revisiting old architectural choices.

### Rule Entropy Analysis - Detect Stale Rules

```
mcp__daem0nmcp__evolve_rule(project_path="/path/to/project")  # All rules
mcp__daem0nmcp__evolve_rule(rule_id=5, project_path="/path/to/project")  # Specific
```

Cross-references rule triggers against code index and outcome history. Produces staleness scores (0.0-1.0) and concrete evolution suggestions when the codebase has evolved past a rule's original intent.
...
mcp__daem0nmcp__debate_internal(
    ...
    advocate_position="Use incremental migrations",
    challenger_position="Use single schema rebuild",
    project_path="/path/to/project"
)
```

Runs structured advocate/challenger debate where every argument must cite memory evidence. Convergence detection halts when positions stabilize. Synthesis is inscribed as a learning memory.

**When to use cognitive tools:**
- `simulate_decision`: Before revisiting an old architectural choice
- `evolve_rule`: Periodically, or when rules seem to be blocking valid work
- `debate_internal`: When weighing two competing approaches with unclear tradeoffs

## Auto-Zoom Retrieval (v6.0.0)

The Daem0n's search adapts to query complexity:
- **SIMPLE** queries → vector-only search (fast)
...
## Background Dreaming (v6.0.0)

The Daem0n thinks during idle periods (60s inactivity):
- Re-evaluates `worked=false` decisions against current evidence
- Suspends immediately when you return
- Dream insights persisted as learning memories

## The Bottom Line

**Memory tools exist. Use them correctly.**

Check context. Record decisions. Track outcomes. Link related memories. Use cognitive tools when facing complex decisions.
